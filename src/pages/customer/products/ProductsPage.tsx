...
import axios from 'axios';
import { useNavigate } from 'react-router-dom';
import { ShoppingCart, Search, Heart, Star, Filter, Package, Sparkles } from 'lucide-react';
import { useCartStore } from '../../../store/cartStore';
import toast from 'react-hot-toast';
      ...
      <div className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              onClick={() => navigate('/customer/cart')}
              className="relative bg-gradient-to-br from-indigo-500 to-purple-600 text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all flex items-center gap-2 font-semibold"
            >
              <ShoppingCart className="w-5 h-5" />
              <span>Cart</span>
              {cartItemsCount > 0 && (
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white shadow-sm"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={selectedCategory}
                onChange={(e) => setSelectedCategory(e.target.value)}
                className="px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white shadow-sm font-medium text-gray-900"
              >
                <option value="">All Categories</option>
                {categories.map((cat) => (
                  <option key={cat} value={cat}>
              ...
              {filteredProducts.map((product) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Product Info */}
                    <div className="p-5">
                      <span className="inline-block text-xs bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full font-semibold mb-3">
                        {product.category}
                      </span>
                      ...
                      {product.stockQuantity > 0 ? (
                        <button
                          onClick={() => handleAddToCart(product)}
                          disabled={inCart >= product.stockQuantity}
                          className={`w-full py-3 rounded-xl font-semibold transition-all transform active:scale-95 flex items-center justify-center space-x-2 ${
                            inCart >= product.stockQuantity
                              ? 'bg-gray-300 text-gray-600 cursor-not-allowed'
                              : 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:from-indigo-700 hover:to-purple-700 shadow-lg hover:shadow-xl'
                          }`}
                        ...
                        <button
                          disabled
                          className="w-full py-3 bg-red-100 text-red-600 rounded-xl font-semibold cursor-not-allowed"
                        >
                          Out of Stock
                        </button>
                      )}
